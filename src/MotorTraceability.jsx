...
import { Wrench, Package, ShieldCheck, FileText, CheckCircle2, Clock, Calendar, Search, Download, X, PieChart, AlertCircle, Filter, Users, Trash2, BarChart3, Activity, TrendingUp, AlertTriangle, Upload, ChevronUp, ChevronDown } from 'lucide-react';
// eslint-disable-next-line no-unused-vars
...
/**
 * MotorTraceability Component
 */
// eslint-disable-next-line no-unused-vars
const MotorTraceability = ({ workOrders = [], programs = [], traceabilityData = [], setTraceabilityData, registeredUsers = [], currentUser = {}, permissions = {}, showToast, syncToDisk, handleFileDownload, handleFilePreview, addNotification, deliveryPlanning = [] }) => {
  ...
  const handleApproval = (id, action) => {
    ...
    if (action === 'approve') {
      if (['Pending Program Owner', 'Pending Program Head'].includes(entry.status)) {
        ...
        addNotification(entry.submittedBy, `Your ${entry.type} record for ${entry.modelNumber} has been fully approved.`, entry.id);
        ...
        addNotification('Program Owner', `${entry.type} record for ${entry.modelNumber} approved by ${entry.status.replace('Pending ', '')}. Pending your approval.`, entry.id);
      }
    } else {
      nextStatus = 'Rejected';
      historyAction = 'Record Rejected';
      addNotification(entry.submittedBy, `Your ${entry.type} record for ${entry.modelNumber} was rejected.`, entry.id);
    }
  ...
  return (
    <div className="page-view module-shell">
      <div className="sub-navigation po-sub-nav">
        ...
        <button className={activeTab === 'proto' ? 'active' : ''} onClick={() => setActiveTab('proto')}><Package size={16} /> Proto</button>
        <button className={activeTab === 'validation' ? 'active' : ''} onClick={() => setActiveTab('validation')}><ShieldCheck size={16} /> Validation</button>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'proto' && (
          <motion.div key="proto" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
            <div className="card table-container">
              <div className="card-header"><h3>PDI Approvals</h3></div>
              ...
              <table className="enterprise-table">
                ...
                <tbody>
                  {traceabilityData.filter(e => e.type === 'PDI').map(entry => {
                    const roles = currentUser.roles || [];
                    const isProtoFH = entry.status === 'Pending Proto Functional Head' && roles.includes('Functional Head') && currentUser.domain === 'Prototyping';
                    const isProgramOwner = (roles.includes('Program Owner') || roles.includes('Program Head')) || (entry.status === 'Pending Program Owner' && (roles.includes('Functional Head') && currentUser.domain === 'Program'));
                    const isDirectMatch = roles.includes(entry.status.replace('Pending ', ''));
                    const isAdmin = roles.includes('Administrator');

                    // If assigned to a specific user, only that user (or an Admin) can approve.
                    // Otherwise, any user with the correct role/domain context can approve.
                    const isAssignedToMe = entry.assignedTo === currentUser.username;
                    const isAssignedToOther = entry.assignedTo && entry.assignedTo !== currentUser.username;
                    const canApprove = entry.status.includes('Pending') && (isAdmin || isAssignedToMe || (!isAssignedToOther && (isProtoFH || isProgramOwner || isDirectMatch)));
                    ...
                    return (
                      <tr key={entry.id}>
                        ...
                        <td>{entry.actualSerialNo}</td>
                        <td>
                          <div style={{ display: 'flex', gap: 'var(--space-xs)' }}>
                            {entry.pdiReport ? (
                              <>
                                <button className="btn-icon-only" onClick={() => handleFilePreview(entry.pdiReport)} title="Preview PDI"><Search size={14} /></button>
                                <button className="btn-icon-only success" onClick={() => handleFileDownload(entry.pdiReport)} title="Download PDI"><Download size={14} /></button>
                              </>
                            ) : (
                              <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-sub)' }}>—</span>
                            )}
                            {canApprove && (
                              <button className="btn-icon-only" style={{ color: 'var(--accent)' }} onClick={() => { setUploadingEntry({ id: entry.id, type: 'PDI', field: 'pdiReport' }); fileInputRef.current?.click(); }} title="Upload PDI Report"><Upload size={14} /></button>
                            )}
                          </div>
                        </td>
                        <td>
                          <span className={`pill-badge ${entry.status === 'Approved' ? 'emerald' : 'amber'}`} style={{ fontSize: 'var(--fs-xs)' }}>
                            {entry.status}
                          </span>
                          {(() => {
                            const reassignedInfo = getReassignedInfo(entry);
                        ...
                        <td>
                          <div style={{ display: 'flex', gap: 'var(--space-xs)' }}>
                            {roles.some(r => ['Administrator', 'Developer', 'Program Owner', 'Program Head'].includes(r)) && (
                              <button className="btn-icon-only" style={{ color: 'var(--rose-text)' }} onClick={() => handleDeleteEntry(entry.id)} title="Delete Entry"><Trash2 size={14} /></button>
                            )}
                            {canApprove && (
                              <>
                                <button className="btn-icon-only" style={{ color: 'var(--emerald-text)' }} onClick={() => handleApproval(entry.id, 'approve')} title="Approve"><CheckCircle2 size={14} /></button>
                                <button className="btn-icon-only" style={{ color: 'var(--rose-text)' }} onClick={() => handleApproval(entry.id, 'reject')} title="Reject"><X size={14} /></button>
                                <button className="btn-icon-only" style={{ color: 'var(--accent)' }} onClick={() => setReassigningEntry(entry)} title="Reassign Role"><Users size={14} /></button>
                              </>
                            )}
                          </div>
                        </td>
                  ...
                  {traceabilityData.filter(e => e.type === 'PDI').length === 0 && <tr><td colSpan="5" className="empty-msg">No records submitted yet.</td></tr>}
                </tbody>
              </table>
            </div>
          </motion.div>
        ...
        {activeTab === 'validation' && (
          <motion.div key="validation" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
            <div className="card table-container">
              <div className="card-header"><h3>EOL Approvals</h3></div>
              <table className="enterprise-table">
                <thead>
                  <tr>
                    <th>Model / Part</th>
                ...
                <tbody>
                  {traceabilityData.filter(e => e.type === 'EOL').map(entry => {
                    const roles = currentUser.roles || [];
                    const isValFH = entry.status === 'Pending Validation Functional Head' && roles.includes('Functional Head') && currentUser.domain === 'Validation';
                    const isProgramOwner = (roles.includes('Program Owner') || roles.includes('Program Head')) || (entry.status === 'Pending Program Owner' && (roles.includes('Functional Head') && currentUser.domain === 'Program'));
                    const isDirectMatch = roles.includes(entry.status.replace('Pending ', ''));
                    const isAdmin = roles.includes('Administrator');
                    const isAssignedToMe = entry.assignedTo === currentUser.username;
                    const isAssignedToOther = entry.assignedTo && entry.assignedTo !== currentUser.username;
                    const canApprove = entry.status.includes('Pending') && (isAdmin || isAssignedToMe || (!isAssignedToOther && (isValFH || isProgramOwner || isDirectMatch)));
                      ...
                      <tr key={entry.id}>
                        ...
                        <td>
                          <div style={{ display: 'flex', gap: 'var(--space-xs)' }}>
                            {entry.eolReport ? (
                              <>
                                <button className="btn-icon-only" onClick={() => handleFilePreview(entry.eolReport)} title="Preview EOL"><Search size={14} /></button>
                                <button className="btn-icon-only success" onClick={() => handleFileDownload(entry.eolReport)} title="Download EOL"><Download size={14} /></button>
                              </>
                            ) : (
                              <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-sub)' }}>—</span>
                            )}
                            {canApprove && (
                              <button className="btn-icon-only" style={{ color: 'var(--accent)' }} onClick={() => { setUploadingEntry({ id: entry.id, type: 'EOL', field: 'eolReport' }); fileInputRef.current?.click(); }} title="Upload EOL Report"><Upload size={14} /></button>
                            )}
                          </div>
                        </td>
                        <td>
                  ...
                  {traceabilityData.filter(e => e.type === 'EOL').length === 0 && <tr><td colSpan="5" className="empty-msg">No EOL records submitted yet.</td></tr>}
                </tbody>
              </table>
            </div>
          </motion.div>
...
export default MotorTraceability;